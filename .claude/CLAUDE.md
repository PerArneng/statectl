...
- `src/statectl/_engine_error.py` — `EngineConfigurationError`, `UnknownDependencyError`, `DuplicateNodeError`.
- `src/statectl/_interfaces/<capability>/` — ABCs for side-effecting capabilities (e.g. `fs/`, `logger.py`) plus the typed error hierarchy in `<capability>_errors.py` (e.g. `fs_errors.py`, `process_errors.py`). Value objects tightly coupled to an ABC (e.g. `FileEntry`, `ProcessResult`) live in the same file as that ABC, not a separate file.
...
- **No stdlib IO in state changers or modules.** Any filesystem/network/process/clock/env call goes behind an interface in `src/statectl/_interfaces/` with a real impl in `src/statectl/_modules/`. See the `new-capability` skill.
